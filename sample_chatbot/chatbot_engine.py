...
from sample_chatbot.agent_utils import TrimConversationHistoryMiddleware, UserRequestLoggingMiddleware
from sample_chatbot.agent_tools import UserContext, data_query, deep_query, knowledge_query, metadata_query
from sample_chatbot.chatbot_utils import setup_user_details
...
class ChatbotEngine:
    def __init__(
        ...
        if self.vector_store and self.kb_description:
            ...
            The knowledge base contents are: {self.kb_description}. Use this knowledge base when the user is asking about anything related to those contents.
            ...
            if mode == "messages":
                ...
                if isinstance(message_chunk, AIMessageChunk) and message_chunk.content:
                    ...
                    if isinstance(message_chunk.content, list):
                        # logging.info("Message chunk content is a list: " + str(message_chunk.content))
                        # logging.info("List items: " + str([item for item in message_chunk.content]))
                        content = "".join([item.get("text", "") for item in message_chunk.content])
                        # logging.info("Joined chunk: " + content)
                    else:
