...
const Results = ({ results, setResults, setCurrentQuestion, setQuestionType }) => {
  ...
  const [selectedContextTables, setSelectedContextTables] = useState({ used: [], unused: [], vql: '' });
  ...
  const handleContextIconClick = (tables, vql) => {
    ...
    setSelectedContextTables({ used: usedTables, unused: unusedTables, vql: vql });
  ...
  const navigateRelatedQuestion = (resultIndex, direction) => {
    ...
    const questionElement = document.querySelector(`[data-result-index="${resultIndex}"] .related-question-text`);
    if (questionElement) {
      questionElement.classList.remove('slide-in-left', 'slide-in-right');
      questionElement.classList.add(direction === 'next' ? 'slide-in-right' : 'slide-in-left');
        ...
        return (
          <div className="d-flex flex-row align-items-center">
            <OverlayTrigger
              placement="left"
              delay={{ show: 250, hide: 400 }}
              overlay={(props) => renderTooltip(props, result.questionType === "deep_query" ? "DeepQuery" : "Denodo")}
            >
              <img 
                src="favicon.ico"
                alt={result.questionType === "deep_query" ? "DeepQuery Icon" : "Denodo Icon"}
                width="20" 
                height="20" 
            ...
            {result.questionType === "deep_query" && result.deepquery_metadata && (
              <OverlayTrigger
                placement="left"
                delay={{ show: 250, hide: 400 }}
                overlay={(props) => renderTooltip(props, "Generate PDF Report")}
              >
                <img 
                  src="pdf.svg" 
  ...
  const renderModalContent = (result) => {
    ...
    switch (result.questionType?.toLowerCase()) {
      ...
      case "deep_query":
        return (
          <div>
            <p><strong>Source:</strong> Denodo DeepQuery</p>
            {result.deepquery_metadata && (
              <>
                <p><strong>Planning LLM:</strong> {result.deepquery_metadata.planning_provider && result.deepquery_metadata.planning_model ? `${result.deepquery_metadata.planning_provider}/${result.deepquery_metadata.planning_model}` : "N/A"}</p>
                <p><strong>Execution LLM:</strong> {result.deepquery_metadata.executing_provider && result.deepquery_metadata.executing_model ? `${result.deepquery_metadata.executing_provider}/${result.deepquery_metadata.executing_model}` : "N/A"}</p>
                <p><strong>Number of tool calls:</strong> {result.deepquery_metadata.tool_calls ? result.deepquery_metadata.tool_calls.length : "N/A"}</p>
              </>
            )}
            {result.total_execution_time && <p><strong>Execution time:</strong> {result.total_execution_time}s</p>}
          </div>
        );
      case "metadata":
        return (
      ...
      default:
        return (
          <div>
            ...
            <p><strong>Model:</strong> {result.chatbot_llm || "N/A"}</p>
          </div>
        );
    }
  };
  ...
  const renderTables = (tables, vql, icons = null) => {
    ...
    return (
      <>
        <div style={{ backgroundColor: 'transparent', padding: '1rem 0', marginBottom: '0' }}>
        <div className={`mb-2 d-flex ${hasTableButtons ? 'justify-content-between' : 'justify-content-end'} align-items-center`}>
          {hasTableButtons && (
            <div className="context-badges-container">
              {/* Show only used tables */}
              {usedTables.map((table, index) => renderTableButton(table, index, true))}
              ...
              {unusedTables.length > 0 && (
                <OverlayTrigger
                  placement="top"
                  delay={{ show: 250, hide: 400 }}
                  overlay={(props) => renderTooltip(props, "View all searched tables")}
                >
                  <Button
  ...
  const renderRelatedQuestions = (result, resultIndex) => {
    const hasRegularQuestions = result.relatedQuestions && result.relatedQuestions.length > 0;
    const hasDeepQueryQuestions = result.relatedQuestionsDeepQuery && result.relatedQuestionsDeepQuery.length > 0;
    
    if (!hasRegularQuestions && !hasDeepQueryQuestions) return null;

    const currentIndex = currentQuestionIndices[resultIndex] || 0;
    const showCarouselArrows = hasRegularQuestions && result.relatedQuestions.length > 1;
    ...
    return (
      <div className="d-flex flex-column align-items-start mt-3">
        ...
        {hasRegularQuestions && (
          <div className="w-100 mb-2">
            <Button
              ...
              onClick={() => {
                setSelectedResult(result);
                handleRelatedQuestionClick(result.relatedQuestions[currentIndex]);
              }}
            >
        ...
        {hasDeepQueryQuestions && result.relatedQuestionsDeepQuery.map((q, i) => (
          <Button
            key={`deep_query-${i}`}
          ...
          >
            <div className="d-flex align-items-center justify-content-between w-100">
              <span className="text-white flex-grow-1" style={{ fontWeight: '500' }}>{q}</span>
        ...
        <React.Fragment key={index}>
          ...
          <div className="w-100 d-flex justify-content-center mb-3">
            <div className="w-70 d-flex justify-content-end">
              <Card style={{ backgroundColor: 'transparent', borderRadius: '1.25em', color: '#112533', boxShadow: '0 4px 12px rgba(0, 0, 0, 0.1)', maxWidth: '80%', width: 'fit-content' }}>
                <Card.Body>
                  <Card.Text>
                    {result.question}
                  </Card.Text>
          ...
          <div className="w-100 d-flex justify-content-center mb-3">
            ...
            <Card className={`w-100 ${result.isLoading ? 'card-loading-pulse' : ''}`} style={{ backgroundColor: 'transparent', color: '#112533', borderRadius: '1.25em', boxShadow: '0 4px 12px rgba(0, 0, 0, 0.1)' }} data-result-index={index}>
                  ...
                  {result.queryPhase === "query" && (
                    <Card.Text className="query-loading-text">
                      <div className="d-flex align-items-start">
                        <Spinner size="sm" animation="border" className="me-2 mt-1" />
                        <span>Querying the AI SDK: <strong>{result.intermediateQuery}</strong></span>
                      </div>
                    </Card.Text>
                  )}
                  ...
                  {result.queryPhase === "complete" && (
                    <>
                      ...
                      {(result.questionType === "data") ? 
                        renderTables(result.tables_used, result.vql, getIcon(result)) : 
                                                  (
                            <>
                              <div style={{ backgroundColor: 'transparent', padding: '1rem 0', marginBottom: '0' }}>
      ...
      >
        ...
        <Modal.Body>
          {selectedContextTables.used.length > 0 && (
            <div className="mb-4">
              <h6 className="text-white mb-3">
                <strong>Used in Query ({selectedContextTables.used.length})</strong>
              </h6>
              <div className="context-badges-container">
                {selectedContextTables.used.map((table, index) => {
                  ...
                  return catalogUrl ? (
                    ...
                    <Button 
                      ...
                      size="sm"
                      className="me-2 mb-2 d-inline-flex align-items-center"
                      style={{ backgroundColor: '#143142', borderColor: '#143142' }}
                    >
                      {table.replace(/"/g, '')}
                    </Button>
                  );
          ...
          {selectedContextTables.unused.length > 0 && (
            <div className="mb-3">
              <h6 className="text-white mb-3">
                <strong>Searched but Not Used ({selectedContextTables.unused.length})</strong>
              </h6>
              <div className="context-badges-container">
          ...
          <div className="mt-4 pt-3 border-top border-secondary">
            <small className="text-white">
              ...
              while grey tables were found in the database but found not relevant to the question.
            </small>
          </div>
        </Modal.Body>
      </Modal>
